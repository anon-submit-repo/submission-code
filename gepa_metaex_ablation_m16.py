...
bench=sys.argv[1]; MPOP=int(sys.argv[2]); COMPACT=sys.argv[3]=="1"; ROUNDS=int(sys.argv[4]) if len(sys.argv)>4 else 60
method="explorer_na"
...
def rollout(prog,n=20):
    ...
    if COMPACT:
        # compact-fail: only the failing cases, capped, framed as recurring error evidence
...
OPS=["ADD_RULE: add a general procedural rule for the top failure cluster",
     ...
     "ADD_NEGATIVE: add an explicit anti-pattern (what NOT to do) for the top failure",
     "ADD_EXAMPLE_ABSTRACTION: distill one failed case into a general (non-instance) heuristic",
     "REORDER: reorder existing rules so the most decisive check comes first",
     "TIGHTEN_SCOPE: narrow an over-broad rule that fires on wrong cases",
     "BROADEN_SCOPE: generalize an over-narrow rule that misses near-variants",
     "ADD_PRIVACY_RULE: add a rule about redacting/withholding sensitive user info",
     "ADD_GROUNDING: require citing/grounding the answer in retrieved evidence",
     "ADD_ABSTAIN: add a rule to abstain/hedge when evidence is insufficient",
     "MERGE_RULES: consolidate two overlapping rules into one crisper rule",
     "CLARIFY_AMBIGUITY: add a rule to resolve the most common ambiguous-input case"]
LENSES=OPS
...
print(f"RESULT_ABL bench={bench} MPOP={MPOP} COMPACT={COMPACT} "+json.dumps(curve))
