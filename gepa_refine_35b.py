...
bench=sys.argv[1]; method=sys.argv[2]; ROUNDS=int(sys.argv[3]) if len(sys.argv)>3 else 60
...
def rollout(prog,n=20):
    ...
    for d in random.sample(train,min(n,len(train))):
        ...
        L.append(f"[{sc:.2f}] gold={str(gold)[:22]} in={str(getattr(d,INP))[:70]}")
    return "\n".join(L)

# per-method refine prompt style
LENSES=["fix the single most common failure","add a precise rule for confusable cases",
        "add a verification step before finalizing","tighten output form","remove a rule that hurts","add step-by-step procedure"]
def propose(cur, fb, r):
    base=f"CURRENT skill:\n{cur}\n\n" if cur else ""
    lens=LENSES[(r-1)%len(LENSES)]
    if method=="oneshot":
        return gen(f"Write ONE natural-language skill(200-400w) that helps a model solve {PHRASE}. Base it on these example tasks.\n{fb}\nReturn only the skill.")
    if method=="skillopt":
    ...
    if method=="gepa":
        return gen(f"You are GEPA, a reflective prompt optimizer for {PHRASE}. {base}Reflect on rollout feedback and evolve ONE improved natural-language instruction/skill(200-400w).\n{fb}\nReturn only the skill.")
    # explorer_na (MetaEx)
...
open(f"{SKILLS_DIR}/{bench}_{method}.md","w").write(best_skill or "")
print(f"SAVED_SKILL {SKILLS_DIR}/{bench}_{method}.md ({len(best_skill or '')} chars, val={best_val})")
print(f"RESULT_35B bench={bench} method={method} val={best_val}")
print("RESULT_REFINE "+method+" "+bench+" "+json.dumps(curve))
