...
import sys, os, json, dspy, re, random, time
...
bench=sys.argv[1]; MPOP=int(sys.argv[2]); COMPACT=sys.argv[3]=="1"; BOTH=os.environ.get("BOTH","0")=="1"; ROUNDS=int(sys.argv[4]) if len(sys.argv)>4 else 60
method="explorer_na"
...
def rollout(prog,n=20):
    ...
    if COMPACT and BOTH:
        # compact-both: failures as recurring-error evidence + a few successes as PROTECTED anchors
        fpart="RECURRING FAILURES (fix these):\n"+"\n".join(misses[:10])
        spart="\n\nALREADY WORKING (do NOT break these; keep the rules that make them pass):\n"+"\n".join(hits[:6])
        return fpart+spart
    if COMPACT:
...
print(f"RESULT_ABL bench={bench} MPOP={MPOP} COMPACT={COMPACT} BOTH={BOTH} "+json.dumps(curve))
