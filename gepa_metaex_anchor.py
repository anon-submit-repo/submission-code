...
ANCHOR=sys.argv[5] if len(sys.argv)>5 else "none"
assert ANCHOR in ("none","evoseed","skillopt","trace2skill","gepa"), ANCHOR
...
if ANCHOR!="none":
    # anchor: let base generator G author an initial skill from one rollout, use as starting point
    _fb0=rollout(M()); queries+=20
    if ANCHOR=="skillopt":
        _seed=gen(f"You are the SkillOpt OPTIMIZER for {PHRASE}. Reflect on failures as a minibatch; propose consolidated add/delete/replace edits into ONE skill(200-400w).\n{_fb0}\nReturn only the skill.")
    elif ANCHOR=="trace2skill":
        _seed=gen(f"You are Trace2Skill for {PHRASE}. Distill trajectory-level lessons into ONE skill(200-400w).\n{_fb0}\nReturn only the skill.")
    elif ANCHOR=="gepa":
        _seed=gen(f"You are GEPA, a reflective prompt optimizer for {PHRASE}. Starting from a base instruction, reflect on the rollout feedback and evolve ONE improved natural-language instruction/skill(200-400w).\n{_fb0}\nReturn only the skill.")
    else:
        _seed=gen(f"EvoSkill seed-evolution for {PHRASE}. Author ONE skill(200-400w).\n{_fb0}\nReturn only the skill.")
    if _seed:
        _v=evalp(wrap(_seed)); queries+=len(val)
        # anchor becomes starting skill (like closed tab:anchor: explorer conditioned on base G's sel-best)
        best_skill=_seed; best_val=_v
...
print(f"RESULT_ABL bench={bench} MPOP={MPOP} MODE={MODE} ANCHOR={ANCHOR} "+json.dumps(curve))
